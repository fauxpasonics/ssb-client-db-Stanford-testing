...
CREATE    VIEW [dbo].[vwCRMLoad_TicketTransactions_archive] AS 
...
SELECT
	...
	, CAST(STUFF(
               (SELECT      ',' + EVENT  + ''
               FROM      TK_ODET AS subtkOdet (NOLOCK) JOIN TK_ODET_EVENT_ASSOC tkOdetEventAssoc (NOLOCK) ON 
...
--WHERE [tkOdet].[I_DATE] > DATEADD(YEAR, -3, GETDATE())	
--AND t.order_line_id__c = 'F15:514186:3'
--AND [tkOdet].[SEASON] = 'F15' AND [tkOdet].[ZID] = '514186:3'
