...
-- 06/26/2018: Keegan Schmitt
-- Change notes: Changed the address primary and phone primary load to null out **

-- Peer reviewed by: Scott Sales
-- Peer review notes:
-- Peer review date:6/27/2018
...
CREATE VIEW [etl].[vw_Load_DimCustomer_SFDCAccount]
AS
...
--base set
		...
		'Stanford SFDC' AS [SourceDB]
		, 'Stanford PC_SFDC Account' AS [SourceSystem]
		...
		, NULL AS [MiddleName]
		, A.LastName [LastName]
		, a.Name + ' ' + ISNULL(a.suffix__c,'') [FullName]
		, suffix__c AS [Suffix]
		--, c.name_title as [Title]

		/*AddressPrimary*/
            , CASE WHEN LEFT(ISNULL(REPLACE(PersonMailingStreet, '*', ''),BillingStreet),1) = '.' THEN RIGHT(ISNULL(REPLACE(PersonMailingStreet, '*', ''),BillingStreet),LEN(ISNULL(REPLACE(PersonMailingStreet, '*', ''),BillingStreet))-1)
			ELSE ISNULL(REPLACE(PersonMailingStreet, '*', ''),BillingStreet) END AS [AddressPrimaryStreet]
			, NULL AS [AddressPrimarySuite]
			, REPLACE(ISNULL(PersonMailingCity,BillingCity), '*', '') AS [AddressPrimaryCity] 
			, ISNULL(PersonMailingState,BillingState)  AS [AddressPrimaryState] 
			, REPLACE(ISNULL(PersonMailingPostalCode,[BillingPostalCode]), '*', '') AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, REPLACE(LEFT(a.[DS_Preferred_Phone__c],25), '*', '') AS [PhonePrimary]
			, LEFT(a.PersonHomePhone,25) AS [PhoneHome]
			, LEFT(a.PersonMobilePhone,25) AS [PhoneCell]
			, LEFT(a.[Business_Phone__pc],25) AS [PhoneBusiness]
			...
			, LEFT(a.[Paciolan_ID__c], 100) AS [ExtAttribute5] 
			...
			, CASE WHEN (SELECT value FROM [dbo].[fn_Split](a.[SSB_CRMSYSTEM_SSID_Paciolan__c],', ') WHERE position = 1) IS NOT NULL
				THEN 'Paciolan-' + (SELECT value FROM [dbo].[fn_Split](a.[SSB_CRMSYSTEM_SSID_Paciolan__c],', ') WHERE position = 1) 
				WHEN (SELECT value FROM [dbo].[fn_Split](a.[Paciolan_ID__c],', ') WHERE position = 1) IS NOT NULL
				THEN 'Paciolan-' + (SELECT value FROM [dbo].[fn_Split](a.[Paciolan_ID__c],', ') WHERE position = 1)
				ELSE NULL
				END AS customer_matchkey
			FROM Stanford_Reporting.prodcopy.account a (NOLOCK)
			WHERE a.isdeleted = 0
			AND a.CopyLoadDate > DATEADD(DAY,-5,GETDATE()) 
