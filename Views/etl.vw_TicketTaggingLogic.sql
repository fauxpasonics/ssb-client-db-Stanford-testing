...
GO
	CREATE VIEW [etl].[vw_TicketTaggingLogic]
		...
		-- DimCustomer --
		, CASE WHEN ttl.logic LIKE 'Customer.%' OR ttl.logic LIKE '% Customer.%' OR ttl.logic LIKE '%(Customer.%' THEN '
		JOIN dbo.DimCustomer Customer (NOLOCK)
			on TicketSales.DimTicketCustomerID = Customer.DimCustomerID' ELSE '' END
