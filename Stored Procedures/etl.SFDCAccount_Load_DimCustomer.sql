...
CREATE PROCEDURE [etl].[SFDCAccount_Load_DimCustomer]
...
-- DROP TABLE #tmpETL
SELECT * INTO #tmpETL FROM etl.[vw_SFDCAccount_Load_DimCustomer]
...
(
		SourceDB, SourceSystem, SourceSystemPriority, SSID, NameDirtyHash, NameIsCleanStatus, FullName,Prefix, FirstName,LastName
		/*, CustomerType, CustomerStatus, AccountType, AccountRep,  Birthday, Gender, MergedRecordFlag, MergedIntoSSID,  MiddleName,  Suffix*/
...
WHEN MATCHED AND isnull(mySource.customer_matchkey,-1) <> isnull(myTarget.customer_matchkey, -1)
...
THEN UPDATE SET 
	...
	, mytarget.matchkey_updatedate	= @RunTime
...
THEN UPDATE SET 
	...
	--SELECT a.IsDeleted
	--SELECT COUNT(*) 
	FROM Stanford_Reporting.ProdCopy.Account a 
	INNER JOIN dbo.DimCustomer b ON a.id = b.SSID AND b.SourceSystem = 'Stanford PC_SFDC Account'
